...
inline void setAutomaticRetryAttempts(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const size_t computerSystemIndex, const uint32_t retryAttempts)
{
...
inline void setAutomaticRetry(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const size_t computerSystemIndex, const std::string& automaticRetryConfig)
{
...
inline void setPowerRestorePolicy(
    const std::shared_ptr<bmcweb::AsyncResp>& asyncResp,
    const size_t computerSystemIndex, std::string_view policy)
{
    ...
    const std::string& systemName)
{
    ...
    // multi-host expects the systemName specified by an entity-manager
    // configuration, i.e 'Yosemite_4_Sentinel_Dome_Slot_1'. Single-host
    // either expects the default 'system' name, or anything specified via
    // -Dbmcweb-redfish-system-uri-name
    ...
    else if (resetType == "Nmi")
    {
        getComputerSystemIndex(
            ...
            [asyncResp{asyncResp}](const size_t computerSystemIndex) {
    ...
    if (hostCommand)
    {
        getComputerSystemIndex(
            asyncResp, systemName,
            [asyncResp{asyncResp}, command](const size_t computerSystemIndex) {
    ...
    const std::string& systemName)
{
    ...
    processComputerSystemGet(asyncResp, systemName);
}
    ...
    const std::string& systemName)
{
    ...
    if (bootSource || bootType || bootEnable)
    {
        getComputerSystemIndex(
            ...
            [asyncResp{asyncResp}, bootSource, bootType,
             bootEnable](const size_t computerSystemIndex) {
    ...
    if (bootAutomaticRetry)
    {
        getComputerSystemIndex(
            asyncResp, systemName,
            [asyncResp{asyncResp},
             bootAutomaticRetry](const size_t computerSystemIndex) {
    ...
    if (bootAutomaticRetryAttempts)
    {
        getComputerSystemIndex(
            asyncResp, systemName,
            [asyncResp{asyncResp},
             bootAutomaticRetryAttempts](const size_t computerSystemIndex) {
    ...
    if (bootTrustedModuleRequired)
    {
        getComputerSystemIndex(
            asyncResp, systemName,
            [asyncResp{asyncResp},
             bootTrustedModuleRequired](const size_t computerSystemIndex) {
    ...
    if (powerRestorePolicy)
    {
        getComputerSystemIndex(
            asyncResp, systemName,
            [asyncResp{asyncResp},
             powerRestorePolicy](const size_t computerSystemIndex) {
    ...
    if (powerMode)
    {
        getComputerSystemIndex(
            asyncResp, systemName,
            [asyncResp{asyncResp},
             powerMode](const size_t computerSystemIndex) {
    ...
    const std::string& systemName)
{
    ...
    getComputerSystemIndex(
        asyncResp, systemName, [asyncResp](const size_t computerSystemIndex) {
            const sdbusplus::message::object_path path(
                "/xyz/openbmc_project/state/host" +
                std::to_string(computerSystemIndex));
            std::string service = "xyz.openbmc_project.State.Host";
